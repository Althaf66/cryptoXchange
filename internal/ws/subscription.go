...
func (sm *SubscriptionManagerStruct) Init() {
	sm.redisClient = redis.NewClient(rediscfg.Options())
...
func (sm *SubscriptionManagerStruct) UserLeft(userID string) {
	...
	// Unsubscribe only splices the slice, so without this the map keeps one
	// empty entry per connection ever made. Anonymous clients get a fresh id
	// each reconnect, which makes that unbounded.
	sm.mutex.Lock()
	delete(sm.subscriptions, userID)
	sm.mutex.Unlock()
}
...
func (sm *SubscriptionManagerStruct) redisCallbackHandler(message, channel string) {
	...
	for _, userID := range userIDs {
		if user := UserManager.GetUser(userID); user != nil {
			if err := user.Emit(payload); err != nil {
				log.Printf("Error emitting to user %s: %v", userID, err)
			}
		}
	}
}
